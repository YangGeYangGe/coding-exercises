...
void recursiveSort(std::vector<unsigned int> &sorted ,std::vector<unsigned int> bucket, unsigned int pow){
    if(bucket.size() == 2){
        ...
        if(aLessB(bucket[0], bucket[1], pow)){
            sorted.push_back(bucket[0]);
            sorted.push_back(bucket[1]);
        } else{
            sorted.push_back(bucket[1]);
            sorted.push_back(bucket[0]);
        }
        ...
        for(auto it = bucket.begin(); it != bucket.end(); ++it){
            ...
            if( (*it) < std::pow(10,pow) ){
                sorted.push_back(*it);
            } else {
                
                unsigned int index = *it;
                while(index > std::pow(10, pow+1)){
                    index /= 10;
                }
                while(index >= 10){
                    index = index % 10;
...
//////////////
    auto sortlambda = [this,&smallbuckets,&mutex](unsigned int start, unsigned int end){
        ...
        for(auto i = start; i <= end; ++i){
            if(smallbuckets[i].size() == 0 || smallbuckets[i].size() == 1 || i == 0){
                continue;
            }
    ...
    unsigned int remain = smallbuckets.size()%numCores;
